...
balls <- ggplot(grille, aes(x = x1, y = x2)) +
  geom_segment(data = df_points[df_points$x1 > mu0[1] & df_points$x1 < mu1[1], ], aes(x = x1, y = x2, xend = ifelse(x1 < (mu0[1] + mu1[1]) / 2, mu0[1], mu1[1]), yend = x2), color = 'grey', linetype = 'solid',size=1) +
  geom_point(aes(x = mu0[1], y = mu0[2]), color = 'black', size = 5) +
  geom_point(aes(x = mu1[1], y = mu1[2]), color = 'black', size = 5) +
  geom_raster(aes(fill = eta_value > 0.5), alpha = 0.2) +
 ...
 # geom_segment(aes(x = mu0[1], y = mu0[2], xend = mu1[1], yend = mu1[2]), color = 'blue') +
  geom_segment(data = df_points[df_points$x1 > mu0[1] & df_points$x1 < mu1[1], ], aes(x = x1, y = x2, xend = (mu0[1] + mu1[1]) / 2, yend = x2), color = 'black', linetype = 'solid',size=0.5) +
